...
describe("Address routes", () => {
  ...
  describe("POST /shipping-address", () => {
    it("should add a new shipping address", async () => {
      db.query
        .mockResolvedValueOnce({ rows: [] }) // Simulate no existing addresses
      ...
      expect(db.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([1, true, true]) // For userId, isShipping, isBilling
      );
    });
  ...
  describe("PATCH /shipping-address/default", () => {
    it("should update default shipping and billing addresses", async () => {
      db.query
        .mockResolvedValueOnce({}) // Reset all
      ...
      expect(db.query).toHaveBeenCalledWith(
        expect.stringContaining(
          `UPDATE shipping_addresses SET is_shipping = false, is_billing = false`
        ),
        [1]
      );
      ...
      expect(db.query).toHaveBeenCalledWith(
        expect.stringContaining(
          `UPDATE shipping_addresses SET is_shipping = true`
        ),
        [1, 1]
      );
      ...
      expect(db.query).toHaveBeenCalledWith(
        expect.stringContaining(
          `UPDATE shipping_addresses SET is_billing = true`
        ),
        [2, 1]
      );
    });
  ...
  describe("DELETE /shipping-address/:id", () => {
    it("should soft delete an address", async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(
        expect.stringContaining(
          "UPDATE shipping_addresses SET is_active = FALSE"
        ),
        ["1", 1]
      );
    });
