...
describe('addressSchema', () => {
  ...
  it('should fail if phoneNumber is invalid', () => {
    ...
    const { error } = addressSchema.validate(input, { abortEarly: false });
    expect(error).toBeDefined();
    expect(error.details[0].message).toBe('Phone number must start with +40 and be a valid mobile number');
  });
  ...
  it('should fail if postalCode is invalid', () => {
    ...
    const { error } = addressSchema.validate(input, { abortEarly: false });
    expect(error).toBeDefined();
    expect(error.details[0].message).toBe('Must be a valid 6-character post code');
  });
