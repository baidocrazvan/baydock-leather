...
export async function getOrderDetails(orderId, userId = null) {
  ...
  const ownershipCheck = userId
    ? await db.query(`SELECT id FROM orders WHERE id = $1 AND user_id = $2`, [
        orderId,
        userId,
      ])
  ...
  return {
    ...
    billingAddress: {
      ...
      country:
        result.rows[0].billing_country || result.rows[0].shipping_country,
      postalCode:
        result.rows[0].billing_postal_code ||
        result.rows[0].shipping_postal_code,
      phoneNumber:
        result.rows[0].billing_phone || result.rows[0].shipping_phone,
    },
